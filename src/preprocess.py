...
def main():
    ...
    parser.add_argument('--seperator', '-sep', type=str, default=',', required=False,
                        help='Seperator to use for reading data if the format is .csv, default is comma(,)')
    ...
    data = preprocessing.read_data(args.data, args.seperator)
    preprocessed_data = strategy.process(data)
    preprocessed_data_path = strategy.save_data(preprocessed_data, args.data, args.strategy)
